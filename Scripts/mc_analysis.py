...
from Functions.optimising_functions import form_new_data_tables
from Functions.sizing_functions import get_airtable
from Functions.cost_functions import calculate_scenarios_iterations, create_iteration_tables, import_excel_data
import warnings
...
def run_analysis(projectID, iter, start_year, revenue_year, end_year):
    split_file = projectID.split("/")
    file = split_file[-1]
    filename = file.strip('.db')
    ...
    pickle_path = os.path.join(parent_path, 'Data', 'mc_analysis', 'scenario_tables_' + filename + '.p')
    ...
    if iter_num > iter_limit:
        ...
        for i in range(repeats):
            ...
            weather_mc_dict, loss_mc_dict, combined_mc_dict, ghi_df = \
                mc_func.run_yield_mc(dc_ordered, input_params, ghi_dict, loss_datatables_split, start_year,
                                     revenue_year, end_year)
            dump_iter(weather_mc_dict, loss_mc_dict, combined_mc_dict, i, filename)
    else:
        weather_mc_dict, loss_mc_dict, combined_mc_dict, ghi_df = \
            mc_func.run_yield_mc(dc_ordered, input_params, ghi_dict, loss_datatables, start_year, revenue_year,
                                 end_year)
    ...
    for scenario in scenario_dict:
        ...
        data_tables_update = (
            data_tables_0, data_tables_1, data_tables_iter[2], data_tables_iter[3], data_tables_iter[4],
    ...
    test = True
    while test:
        tag = 'analysis_dict' + '_' + filename + '_' + str(i) + '.p'
    ...
    file_name = 'analysis_dictionary_' + filename + '.p'
    pickle_path = os.path.join(parent_path, 'Data', 'mc_analysis', file_name)
